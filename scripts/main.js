...
// O código abaixo é do tutorial WebGL2Fundamentals
...
async function main() {
  ...
  const objHref = 'resources/tree_forest.obj';
  const response = await fetch(objHref);
  const text = await response.text();
  const obj = parseOBJ(text);
  const matHref = 'resources/tree_forest.mtl';
  const filename = await fetch(matHref);
  const matTexts = await filename.text();
